...
nodeTemplate.nodeList = [
    ...
    '<a href="#nodediagnostics" class="btn btn-info" rel="tipRight" data-trigger="hover" data-placement="bottom" data-title="为集群中所有节点提供有效的诊断和工作信息"><i class="icon-ambulance"></i> 集群节点诊断</a>',
    ...
    'data-content="<b>IP:</b> <%- node.attributes.transport_address %>.<br/><b>ID:</b> <%- node.id %>" data-html="true" data-title="点击查看节点信息">',
    '<% if (node.attributes.master == true) { %>',
    '<i class="icon-bolt"></i> ',
    '<% } %>',
    '<%= node.attributes.name %></a>',
...
nodeTemplate.nodeInfoModal = [
    ...
    '<tr><td>节点名称:</td><td><%- settings.nodeName %></td></tr>',
    '<tr><td>节点ES版本:</td><td><%- version %></td></tr>',
    '<tr><td>此节点是否是主节点?</td><td><%- settings.nodeMaster %></td></tr>',
    '<tr><td>此节点是否有数据?</td><td><%- settings.nodeData %></td></tr>',
    '<tr><td>集群名称:</td><td><%- settings.clusterName %></td></tr>',
    '<% if(versionUtil.isNewer("0.99.0", cluster.versionNumber.concat)) { %>',
    '<tr><td>节点主机名称:</td><td><%- settings.host %></td></tr>',
    '<% } else { %> ',
    '<tr><td>Hostname:</td><td><%- host %></td></tr>',
    '<tr><td>Logger Prefix:</td><td><%- settings.logPrefix %></td></tr>',
    '<% } %>',
    '<tr><td>HTTP地址:</td><td><%- settings.http_address %></td></tr>',
    '<tr><td>ES文件地址:</td><td><%- settings.pathHome %></td></tr>',
    '<tr><td>日志地址:</td><td><%- settings.logPath %></td></tr>',
...
nodeTemplate.diagnostics = [
    '<div class="well">',
    '<div class="span2 pull-left"><a href="#refreshNodeDiagnostics" class="btn btn-mini"  rel="tipRight" data-placement="bottom" data-html="true" data-title="每 <%- polling/1000 %> 秒更新。<br/>点击立刻更新"><i class="icon-refresh"></i> <%- lastUpdateTime %></a></div>',
    '<div class="pull-right">',

    '<div class="btn-group">',
    '<a href="#selectDiagnosticsNodeModal" id="openNodeSelect" class="btn" rel="tipRight" data-placement="bottom" data-html="true" data-title="Monitoring <%- nodes.length %> of <%- allNodes.length %> nodes."><i class="icon-list-ul"></i> 选择</a>',
    '<a href="#diagnosticsModal" style="visibility:hidden;" data-toggle="modal" role="button" class="btn" rel="tipRight" data-placement="bottom" data-html="true" data-title="Trouble understanding this screen?"><i class="icon-question-sign"></i> Help</a></div>',
    '</div>',
    '<div class="text-center">',
    '<span style="font-size: 28px;">集群节点诊断信息</span>',
    '</div>',
    '</div><!-- well -->',
    ...
    '<thead><tr><td colspan="<%- _.size(nodes)+1 %>" class="lead" style="font-weight: bold;"><i class="icon-th-large"></i> 总览</td></tr></thead>',
    '<tbody>',
    ...
    '</tbody>',
    // FS
    '<thead><tr><td colspan="<%- _.size(nodes)+1 %>" style="font-weight: bold;"><i class="icon-th-large"></i> 文件系统</td></tr></thead>',
    '<tbody>',
    ...
    // NETWORK
    '<thead><tr><td colspan="<%- _.size(nodes)+1 %>" class="lead" style="font-weight: bold;"><i class="icon-th-large"></i> 网络</td></tr></thead>',
    '<tbody>',
    ...
    '<p>选择需要进行诊断的节点（注意：选择过多节点数，可能会导致响应缓慢）</p>',
    '<form  class="form-inline"><select id="selectedNodes" class="selectpicker" data-style="btn-default" data-selected-text-format="count>5" multiple title="选择节点..." data-size="10">',
    ...
    '<% }) %>',
    '</select>',
    '<button class="btn btn-info" id="refreshSelectedNodes">过滤</button>',
    '</form>',
    '</div>',
    '</div>',
    '</div>',
...
nodeTemplate.nodeInfo = [
    ...
    '<div class="well">',
    '<div class="pull-left"><button id="refreshNodeInfo" class="btn btn-mini"  rel="tipRight" data-placement="bottom" data-html="true" data-title="每 <%- polling/1000 %> 秒更新。<br/>点击立刻更新"><i class="icon-refresh"></i> <%- lastUpdateTime %></button></div>',
    //'<div class="pull-left" style="line-height: 13px;font-size: 10px;">Refreshed:<br/><%- lastUpdateTime %></div>',

    '<div id="toolbar" class="pull-right">',
    '<div class="btn-group">',
    '<a href="#nodeInfoModal" role="button" data-toggle="modal" class="btn btn-info" rel="tipRight" data-placement="bottom" data-title="设置信息"',
    '><i class="icon-info-sign"></i></a>',
    '<a href="#killnodemodal" data-toggle="modal" role="button" class="btn btn-info" rel="tipRight" data-placement="bottom" data-title="关闭节点"><i',
    ' class="icon-off"></i>',
    '</a>',
    '</div> <!-- btn group -->',
    '</div> <!-- toolbar --> ',
    ...
    '<div class="text-center">&nbsp;</div>',
    '<table class="table table-condensed table-striped table-bordered">',
    '<tr><td>文件数:</td><td><%- numeral(indices.docs.count).format("0,0") %></td></tr>',
    '<tr><td>文件数 Deleted:</td><td><%- numeral(indices.docs.deleted).format("0,0") %></td></tr>',
    ...
    /* NETWORK */
    '<div class="lead text-left"><i class="icon-th-large"></i> 网络</div>',
    '<div class="row-fluid">',
    ...
    /* FS */
    '<div class="lead text-left"><i class="icon-th-large"></i> 文件系统</div>',
    ...
    '<div class="span4">',
    '<div class="text-center"><strong>磁盘读取次数</strong></div>',
    '<div class="chart-container text-center">',
    ...
    '<div class="span4">',
    '<div class="text-center"><strong>磁盘写入次数</strong></div>',
    '<div class="chart-container text-center">',
