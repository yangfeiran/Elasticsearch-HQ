...
function fs_rules() {
    return [
        ...
        },
        {
            ...
            comment:"不充分的文件合并会导致此值较高<br/>读写缓慢？",
            format:"pct",
...
function action_rules() {
    return [
        ...
        {
            label:"查询耗时:",
            comment:"查询语句过于复杂或者不完整，不充分利用过滤器，缓存RAM不足，或者读写缓慢会导致此值较高",
            format:"ms",
        ...
        {
            label:"获取耗时:",
            comment:"读写缓慢，文件过大，或者获取过多文件会导致此值较高",
            format:"ms",
    ...
    return [
        {
            label:"HTTP连接速率:",
            unit:"/second",
            comment:"同一时间过多的HTTP访问会耗尽内核的连接数，导致服务中断",
            format:"comma",
